...
\begin{tikzcd}
  ...
  C \arrow{r}{\psi} & D \end{tikzcd} & & \begin{tikzcd}
  C \arrow{d}[left]{f} \arrow{r}{\varphi} & F\ C \arrow{d}{F\ f} \\
  D \arrow{r}{\psi} & C \end{tikzcd} \\
  \ & \  &\  \\
...
\end{center}
  ...
  there exists a unique arrow $\varphi : \mu F \rightarrow C$ where $f = \llparenthesis \varphi \rrparenthesis$
  and is called catamorphism.
...
\begin{tikzcd}
  ...
  C \arrow{d}[left]{[(\varphi )]} \arrow{r}{\phi} & F\ C\arrow{d}{F\ \llparenthesis \varphi \rrparenthesis} \\
  \nu F \arrow{r}{out} & F \nu F\end{tikzcd} \\
  \ & \  &\  \\
  $f \circ in = \varphi \circ F\ f \equiv f = \rparen\rbrack \varphi \rrparenthesis$& &
  $out \circ f = F\ f \circ \varphi \equiv f = \lBrack \varphi \rBrack$ \\
\end{tabular}
\end{center}
