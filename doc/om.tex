...
Record Pi (A: Type) :=
    ...
    lambda: (B: A $\rightarrow$ Type) $\rightarrow$ $\forall$ (a: A) $\rightarrow$ B a $\rightarrow$ intro B
    app: (B: A $\rightarrow$ Type) $\rightarrow$ intro B $\rightarrow$ $\forall$ (a: A) $\rightarrow$ B a
    app-lam (B: A $\rightarrow$ Type) (f: $\forall$ (a: A) $\rightarrow$ B a): $\forall$ (a: A) $\rightarrow$ app (fun f) a ==> f a
    lam-app (B: A $\rightarrow$ Type) (p: intro B): fun ($\lambda$ (a: A) $\rightarrow$ app p a) ==> p.
\end{lstlisting}
