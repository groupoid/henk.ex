...
\usepackage{graphicx}
\begin{document}
\title{The Systems Engineering of Consistent Pure Language with Effect Type System
...
The core of CiC is Calculus of Constructions\footnote{T. Coquand, Ǵerard Huet. The calculus of constructions} (CoC)\cite{Coq88}.
The further developing led to Lambda Cube\footnote{H. Barendregt. Lambda Cacluli with Types}\cite{Henk93} and Pure Type Systems (Henk\footnote{Simon Peyton Jones, Erik Meijer. Henk: a typed intermediate language.}\cite{Erik97},Morte\footnote{Gabriel Gonzalez. Haskell Morte Library}).
Pure Type Systems are custom languages based on CoC with single Pi-type and possible other extensions.
The known extensions are ECC\footnote{Zhaohui Luo. An Extended Calculus of Constructions}, ECC with Inductive Types\cite{Ore92}, K-rules\footnote{Gilles Barthe. Extensions of pure type systems.}\cite{Barthe95}.
...
or Pi caclulus (elements are different languages such as Erlang, Ling, Rust).
...
Not all terms from higher languages could be desugared to PTS. As we know we have
limitations on presenting the recursion and we cannot build the J and FunExt withing PTS directly.
But still PTS is very powerful, it's compatible with System F libraries.
The properties of that libraries could be proven within Higher Languages
with Induction and [0,1] Homotopy Core. Then Runtime part could be refined
to PTS and extract targets.
...
Our work is targeted Erlang platform and will be veri OTP specific.
The product is and Erlang/OTP application that can be built-in any
Erlang solution and provide dependent language services to the Erlang environment:
1) typechecking; 2) normalization; 3) extraction. The all parts of Om
compiler are written in Erlang language and target language is Erlang.

\begin{itemize}
\item Level 0 --- vectorized interpreter
\item {\bf Level 1 --- consistent pure type system for typechecking and normalization}
\item Level 2 --- higher language for theorem proving and property checking
\end{itemize}
...
language for {\bf Exe} general purpose language. The overall
size of {\bf Om} language with extractor to Erlang is 263 lines of code.
...
\hline
\tch{1}{c}{b}{Stage} & \tch{1}{c}{b}{LOC} & \tch{1}{c}{b}{Description}\\
\hline
TOKEN   & 54 LOC & Handcoded Tokenizer\\
PARSER  & 81 LOC & Inductive AST Parser\\
...
2) however for running applications in runtime it is enough System F programs or Dependent Types without Fixpoint.
...
runtime parts of specification into PTS$^\infty$ with later extraction to any functional language.
...
1) refined version of typechecker and clean implementation in 200 LOC.
This will make more trust to the core by external institutions.
2) supporting both predicative and impredicative hierarhies of PTS$^\infty$ configuration.
...
3) In the academia PTS$^\infty$ could be used as teaching instrument for logic,
...
2) Build a theory of compilation and erasing from higher languages to PTS$^\infty$.
2) Build a certified interpreter (replace Erlang) in future higher level Cubical language.
