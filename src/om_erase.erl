...
erase({var,{N,I}},D)         -> true = var(N,D),
                                ...
                                case univ(T) of
                                     true  -> {none,T};
                                     false -> {{var,{N,I}},T} end;
erase({"→",{I,O}},D)         -> {none,{star,hierarchy(star(type(I,D)),star(type(O,D)))}};
erase({{"∀",{N,0}},{I,O}},D) -> {none,{star,hierarchy(star(type(I,D)),star(type(O,[{N,norm(I)}|D])))}};
erase({{"λ",{N,0}},{I,O}},D) -> star(type(I,D)),
                                NI = norm(I),
