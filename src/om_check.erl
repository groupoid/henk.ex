...
-define(FUN(Fun), Fun=="λ";Fun=="∀").

freeIn({var,{X,N}}=_V,{{Fun,X},{In,Out}}) when ?FUN(Fun) -> freeIn(_V,In) orelse freeIn({var,{X,N+1}},Out);
freeIn({var,{X,N}}=_V,{{Fun,Y},{In,Out}}) when ?FUN(Fun) -> freeIn(_V,In) orelse freeIn(_V,Out);
...
shift(D,Text,{{Fun,X},{A,B}},C) when ?FUN(Fun) -> {{Fun,X},{shift(D,Text,A,C),shift(D,Text,B,case X of Text->C+1; _->C end)}};
...
subst(X,N,EP,{{Fun,XP},{A,B}}) when ?FUN(Fun)  -> {{Fun,X},{subst(X,N,EP,A),subst(X,case X of XP -> N+1;_->N end,shift(1,XP,EP),B)}};
